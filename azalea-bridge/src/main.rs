...
fn item_kind_name(stack: &ItemStack) -> Option<String> {
    let item = stack.as_present()?;
    if item.is_empty() {
        return None;
    }
    // ItemKind is generated by Azalea's registry macro. Its Debug spelling is
    // the stable PascalCase registry name (for example, OakPlanks), so turn it
    // into the snake_case names already understood by the web UI.
    Some(pascal_to_snake(&format!("{:?}", item.kind)))
}

fn item_slot_json(slot: usize, stack: Option<&ItemStack>, selected: bool) -> Value {
    ...
    let Some(name) = item_kind_name(stack) else {
        return json!({
            "slot": slot,
            "name": null,
            "displayName": null,
    ...
    let count = stack.count().max(0);
    json!({
        "slot": slot,
        "name": name,
        "displayName": display_item_name(&name),
        "count": count,
        "selected": selected
    })
}

fn inventory_snapshot(bot: &Client) -> (Vec<Value>, Option<String>, Option<Value>) {
    let menu = bot.menu();
    ...
    let hotbar = hotbar_range
        .clone()
        .enumerate()
        .map(|(hotbar_slot, protocol_slot)| {
            item_slot_json(
                hotbar_slot,
                menu.slot(protocol_slot),
    ...
    let held = menu
        .slot(hotbar_start.saturating_add(selected))
        .and_then(item_kind_name);

    // A non-player menu represents a server container currently open. Keep
    // this lightweight but include its real contents so the existing game-view
    // panel can render chests, shops, and other menus too.
    let window = if menu.try_as_player().is_some() {
        None
    } else {
        Some(json!({
            "title": "Container",
            "slots": menu
                ...
                .iter()
                .enumerate()
                .map(|(slot, stack)| item_slot_json(slot, Some(stack), false))
                .collect::<Vec<_>>()
        }))
    };
...
fn snapshot(bot: &Client) -> Value {
  ...
  let (hotbar, held_item, window) = inventory_snapshot(bot);
  json!({
        "ev": "snapshot",
        "available": true,
